...
current = 0.1;
Turns = 6;
nParts = 360; % Stueckelung pro windung
...
gyro_mag_rat = 42577000; %gyromagnetic ratio in [Hz/T] to convert from tesla to Hz


NP = 200; % 100 gut Gridaufloesung pro Raumdimension 200 dauert lange
...
hplot = surf(squeeze(xxP(:,:,1)), squeeze(yyP(:,:,1)), squeeze(B(NP/2,:,:)*gyro_mag_rat/(current*1000)));%/0.005*250000)); %tesla/0.005*250000 = Herz
...
colourbarlimit = 130;
caxis([70 colourbarlimit]);%([-0.00002 0.00002]);
w = c.LineWidth;
c.LineWidth = 1.5;
% alpha(.6) %.2)
% alpha(c, .6) %.2)
...
[hplot, c] = contour(squeeze(xxP(:,:,1)), squeeze(yyP(:,:,1)), squeeze(B(NP/2,:,:)*gyro_mag_rat/(current*1000)),'LevelList',[80 90 100 110 115 120],'ShowText','on');%,'DisplayName','frequency offset [Hz]'
